			...
			if (d || e) setTimeout(n, 100);
				...
				j.filter('pre,code').each(function () {
					function r(b, c, e) {
						var h = !1,
							i = a.find('.vg-line');
						c && (a.find('.vg-highlight').removeClass('vg-highlight'), f.remove(d), (k = [])),
							(b = $.type(b) === 'array' ? b : [b]),
							$.each(b, function (b, c) {
								if (k.indexOf(c) <= -1) {
						...
						$.each(o, function (a, b) {
							...
							a.find('.vg-number').click(function (b) {
								...
								(j = parseInt(g.pageNumber.exec(e)[1], 10)),
									b.shiftKey && j ? r(i < j ? i + '-' + j : j + '-' + i, !0) : r(j, b.ctrlKey ? !1 : !0);
								return !1;
							});
										...
										if (this.scrollLeft < w) (w = this.scrollLeft), u.hide();
											...
											(w = this.scrollLeft),
												(x = setTimeout(function () {
													...
													e.css('marginLeft', v),
														u
															.css({ float: 'none', position: 'absolute', left: a - v })
															.show()
															.stop()
															.animate({ left: a });
												}, 500));
			...
			success: function (response) {
				...
				$.each(response.data.tree, function (i) {
					...
					$.each(arr, function (i) {
						...
						if (!exists) {
							...
							if (output !== first) {
								output
									.find('ul li.back')
									.after($('<li class="dir"><a href="#" data-id="' + id + '">' + name + '</a></li>'));
							} else {
								output
									.find('ul li')
									.first()
					...
					output
						.find('ul')
						.append(
							$('<li class="file"><a href="#" data-path="' + path + '" data-id="' + id + '">' + file + '</a></li>')
						);
				...
				_this.container.on('click', 'a', function (e) {
					...
					if (parent.hasClass('file')) {
							...
							$.ajax({
								...
								success: function (response) {
									...
									if ('image' === mimeType.split('/').shift()) {
										el = fileContainer
											.append($('<div class="image"><span class="border-wrap"><img src="" /></span></div>'))
											.appendTo(repo);
										el.find('img')
						...
						if (link[0] !== _this.container.find('h1 a')[0]) link.addClass('active');
						_this.container
							.find('h1 a')
