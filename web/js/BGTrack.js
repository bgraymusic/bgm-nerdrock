...
BG.Track = function (album, bcInfo, bgInfo) {
	...
	for (var prop in bcInfo) {
		this[prop] = bcInfo[prop];
	}
	for (var prop in bgInfo) {
		this[prop] = bgInfo[prop];
	}
...
BG.Track.css = {
	hdr: {
		cont: 'bg-accordion-header',
		name: 'bg-track-name',
		name_text: 'bg-track-name-text',
		jcc_img: 'bg-track-name-jcc-img',
		player: 'bg-track-player',
		controls: {
			cont: 'bg-track-controls',
			play: 'bg-track-play-pause-button',
			shuttle: {
				cont: 'bg-track-shuttle',
				table: 'bg-track-shuttle-table',
				value: 'bg-track-shuttle-value',
				slider: 'bg-track-shuttle-slider',
				max: 'bg-track-shuttle-max',
			},
			vol: {
				cont: 'bg-track-volume',
				table: 'bg-track-volume-table',
				min: 'bg-track-volume-min',
				slider: 'bg-track-volume-slider',
				max: 'bg-track-volume-max',
			},
	...
	body: {
		...
		notes: 'bg-track-notes',
		media: 'bg-track-media',
		jcc: 'bg-track-jcc',
		credits: 'bg-track-credits',
		tab: 'vex-tabdiv',
		score: 'vex-tabdiv',
	},
};
	...
	this.player.jPlayer({
		...
		play: function (event) {
			...
			if (track.album.cont.find('.' + BG.Album.css.meta.follow).prop('checked')) {
				...
				track.album.accordion.accordion('refresh');
			}
...
BG.Track.prototype.buildTrackShuttle = function (shuttle) {
	...
	shuttle.append(
		$('<div/>').addClass(BG.Track.css.hdr.controls.shuttle.max).text($.jPlayer.convertTime(this.duration))
	);
};
...
BG.Track.prototype.buildBody = function () {
	...
	if (this.lyrics) {
		tabs.append(
			$('<li/>').append(
				$('<a/>')
					.attr('href', '#' + this.track_id + 'lyrics')
					.text('Lyrics')
			)
		);
		this.body.append(
			$('<div/>')
				.attr('id', this.track_id + 'lyrics')
				.addClass(BG.Track.css.body.lyr.cont)
	...
	if (this.about) {
		tabs.append(
			$('<li/>').append(
				$('<a/>')
					.attr('href', '#' + this.track_id + 'notes')
					.text('Notes')
			)
		);
		this.body.append(
			$('<div/>')
				.attr('id', this.track_id + 'notes')
				.addClass(BG.Track.css.body.notes)
				.html(this.about)
		);
	}
	...
	if (this.media) {
		tabs.append(
			$('<li/>').append(
				$('<a/>')
					.attr('href', '#' + this.track_id + 'media')
					.text('Media')
			)
		);
		var media = $('<div/>')
			.attr('id', this.track_id + 'media')
			.addClass(BG.Track.css.body.media);
	...
	if (this.jcc && BG.NerdRock.getInstance().badges.hasBadge('j')) {
		tabs.append(
			$('<li/>').append(
				$('<a/>')
					.attr('href', '#' + this.track_id + 'jcc')
					.text('SeaMonkeys')
			)
		);
		var jcc = $('<div/>')
			.attr('id', this.track_id + 'jcc')
			.addClass(BG.Track.css.body.jcc);
		$(this.jcc).each(function () {
			if (this.ytId)
				jcc.append(
					$('<fieldset/>')
						.append($('<legend/>').text(this.label))
	...
	if (this.credits) {
		tabs.append(
			$('<li/>').append(
				$('<a/>')
					.attr('href', '#' + this.track_id + 'credits')
					.text('Credits')
			)
		);
		this.body.append(
			$('<div/>')
				.attr('id', this.track_id + 'credits')
				.addClass(BG.Track.css.body.credits)
				.html(this.credits)
		);
	}
...
BG.Track.markElapsedLyrics = function (player, time) {
	...
	if (!info) {
		for (id in trackInfo) {
			if (trackInfo[id].karaoke == trackId) info = trackInfo[id];
		}
	}
...
BG.Track.registerJQueryUI = function () {
	...
	$('.bg-track-shuttle-slider').each(function () {
		$(this)
			.slider({
				range: 'min',
				min: 0,
				max: Math.floor(BG.Track.getFromElement(this).duration),
				value: 0,
	...
	$('.bg-track-volume-slider').slider({
		...
		animate: 'fast',
		slide: function (event, ui) {
			BG.Track.getFromElement(this)
				.player.data()
				.jPlayer.volume(ui.value / 100);
		},
	});
