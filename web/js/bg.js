var BLOGROOT = "https://blog.briangraymusic.com";
var bc = new Bandcamp();
...
function bgInit() {
  ...
  navigate({
    toptab: $.url().param("toptab"),
    blog: $.url().param("blog"),
    song: $.url().param("song"),
    songtab: $.url().param("songtab"),
  });
}
...
function navigate(params) {
  ...
  if (params["song"]) {
    ...
    $(discography.bgAlbums).each(function () {
      $(this.masterTracks).each(function () {
        if (
          BG.Track.mashTitle(this.title) == BG.Track.mashTitle(params["song"])
        ) {
          track = this;
          return false;
        }
    ...
    $(".bg-album-accordion").each(function () {
      var header = $(this).find(
        ".bg-accordion-header[song=" + BG.Track.mashTitle(params["song"]) + "]"
      );
  ...
  $("#bg-blogframe").attr(
    "src",
    blogroot + (params["blog"] ? params["blog"] : "")
  );
}
