...
Bandcamp.prototype = {
	...
	getBandcampDataNative: function(cb, bandId) {
		this.discography = {};
		this.albumsToFetch = [];
		this.callback = cb;
		var bcObj = this;
		...
		oReq.addEventListener("load", function() {
			bcObj.discography = JSON.parse(this.responseText).discography;
			console.log(bcObj.discography);
			for (var albumIdx in bcObj.discography)
				bcObj.albumsToFetch.push(bcObj.discography[albumIdx].album_id);
			for (var albumIdx in bcObj.discography) {
				var album = bcObj.discography[albumIdx];
				bcObj.getAlbumNative(album);
			}
		});
		oReq.open("GET", Bandcamp.apiURL + Bandcamp.discographyApi + '?key=' + Bandcamp.apiKey + '&band_id=' + bandId);
		oReq.send();
	},
	...
	getAlbumNative: function(album) {
		console.log(album);
		this.albumsToFetch = this.albumsToFetch.filter(function(i) { return i != album.album_id; });
		console.log(this.albumsToFetch);
		if (!this.albumsToFetch.length)
			console.log('Last album');
		return;
		var oReq = new XMLHttpRequest();
		oReq.addEventListener("load", function() {
			albumDetails = this.responseText;
		});
		oReq.open("GET", Bandcamp.apiURL + Bandcamp.albumInfoApi + '?key=' + Bandcamp.apiKey + '&album_id=' + album.album_id);
		oReq.send();
	}
}
